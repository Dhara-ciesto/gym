...
$sql = "SELECT * FROM hr_deviceinfo where portno='".$port_const."'";
...
$device_status = mysqli_query($conn, 'SELECT * FROM hr_device_status LIMIT 1');
if(!$device_status){
...
if (is_resource($connection))
{
    ...
    $status = mysqli_query($conn ,'UPDATE  hr_device_status SET status = 1, updated_at = "'.$date.'"');
...
{
    mysqli_query($conn ,'UPDATE  hr_device_status SET status = 0, updated_at = "'.$date.'"');
    $device_status = mysqli_query($conn, 'SELECT * FROM hr_device_status LIMIT 1');
    if(!$device_status){
