  ...
  <div class="modal-content">
    ...
    <div class="modal-body">
     <?php $permission = unserialize(session()->get('permission')); ?>
      <form role="form" action="{{ url('viewfollowupcall/'.$id) }}" method="POST" >
         {{ csrf_field() }}
        ...
        <div class="form-group">
         <div class="box-tools pull-right">
            <div class="input-group">   
                    <label>Status</label>
                    <select class="form-control select2"  width="100%" id="inqstatus" name="status" onchange="changeinqstatus({{$followupid->inquiriesid}})">
                          <option value="" selected="">Please select status</option>
                          <option value="0"  {{$followupid->status == 0 ? 'selected':''}} >Close Inquiry</option>
                          <option value="1" {{$followupid->status == 1 ? 'selected':''}}>Active</option>
                          <option value="2" {{$followupid->status == 2 ? 'selected':''}}>Confirmed</option>
                          {{-- <option value="3" {{$followupid->status == 3 ? 'selected':''}}>Converted Inquiry</option> --}}
                        </select>
                    </div>
        ...
        <div class="form-group">
          ...
          <div class="col-md-2 col-md-offset-1">
            <label for="CallCompletedBy">Call Completed By</label>
              @php $loginid = Session::get('username'); @endphp
              @if(!$loginid )
             <select class="form-control select2" id="exampleFormControlSelect1" name="callcompletedby">
                @php $loginid = Session::get('username'); @endphp
                
                  @foreach($pocarray as $poc)
                    <option value="{{$poc}}" {{$loginid == $poc ? 'selected':'' }}>{{$poc}}</option> 
                  @endforeach       
            </select>
            @else
            <input  type="text" id="callcompletedby" class="form-control" name="callcompletedby" readonly value="{{$loginid}}"> 
          ...
          <div class="form-group">
            <div class="col-md-6 col-md-offset-2">
              <label for="PackageInterestedIn">Schedule Next Call Date</label>
                ...
                <input  type="date" id="scheduledate" min="<?php echo date("Y-m-d"); ?>" class="form-control" name="scheduledate" placeholder="<?php echo date("d-m-Y"); ?>" value="" autocomplete="off">
            </div>
          </div>
       </div>
    ...
    <div class="modal-footer">
      <div class="form-group">
        ...
        <div class="col-md-2 col-md-offset-8"> <a href="{{ url()->previous() }}" class="btn btn-danger btn-block">Cancel</a>
         </div>
...
function changeinqstatus(inqid){
  var inqstatus = $('#inqstatus').val();
  $.ajax({  

    type:"POST",  
    data: {"_token": "{{ csrf_token() }}","inqid": inqid,"inqstatus":inqstatus},
    url:'{{ url("changeinqstatus") }}', 
