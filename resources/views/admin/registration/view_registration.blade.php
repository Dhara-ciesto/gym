       ...
       @endif
        ...
        <div class="col-md-12">
          <div class="box">
            ...
            <!-- /.box-header -->
            <div class="box-body">
              <form class="form-inline" method="post" action="{{ url('registrationdetails') }}">
                  {{ csrf_field() }}
                  <div class="row"> 
                   <div class="col-md-3" >
                      <div class="">
                        <label>Member Name</label>
                        <select  name="username" class="form-control select2"title="Select Name" data-live-search="true" id="username">
                          <!-- <option value="" selected disabled>Select First Name</option> -->
                          <option value="">Select Name</option>
                          @foreach($users as $user)
                            <option value="{{ $user->id }}"@if(isset($query['username'])) {{$query['username'] == $user->id ? 'selected':''}} @endif>{{ $user->firstname }}  {{ $user->lastname }}</option>
                          @endforeach
                        </select>
                        </div>
                      </div>
                    <div class="col-sm-3">
                      ...
                      <select data-width="180px" name="mobileno" class="form-control select2" id="mobileno" title="Select Mobileno" data-live-search="true" >
                        ...
                        @foreach($users as $user)
                          <option value="{{ $user->id }}" @if(isset($query['mobileno'])) {{$query['mobileno'] == $user->inquiriesid ? 'selected':''}} @endif>{{ $user->phone_no }}</option>
                        @endforeach
                      </select>
                    </div>
                    <div class="col-md-3">
                    ...
                    <div class="col-md-3">
                      <div class="input-group date" id="endate">
                        <label> Date To</label>
                  ...
                  <div class="row"> 
                    <div class="form-group col-sm-3">
                     ...
                     <div class="form-group "style="margin-top:0.9%;">
                      ...
                      <a href="{{ url('registrationdetails') }}" class="btn bg-red">Clear</a>
                    </div>
                  </div>
          ...
          <div class="box">
            <div class="box-header">
              ...
              <table id="example1" class="table table-bordered table-striped dataTable">
                ...
                <tbody>
                  @if($registrationfield)
                    @foreach($registrationfield as $regfields) 
                    <tr>
                      <td>{{ucwords($regfields->firstname)}} &nbsp; {{ucwords($regfields->lastname)}}</td>
                      <td>{{$regfields->phone_no}}</td>
                      <td>{{ date('d-m-Y', strtotime($regfields->starting_date))}}</td>
                      <td>{{$regfields->schemename}}</td>
                      <td>
                        @if($regfields->rstatus == 1)
                          Active
                        @else
                          Deactive
                        @endif
                      ...
                      <td>
                        ...
                        <a href="{{url('viewregistrationdetails/'.$regfields->id)}}" title="View Registration" target="_blank"><i class="fa fa-eye" style="color: #0f184b;"></i></a>
                        <a href="{{url('regconvertmember/'.$regfields->id)}}" title="Convert Into Member"><i class="fa fa-user-plus" style="color: #05BD6C;"></i></a>
                        <a href="{{ route('deletereg', $regfields->id) }}"   onclick="return myFunction();" title="Delete" class="delete" title="Delete"><i class="fa fa-trash"></i></a>
              ...
              {{ $registrationfield->appends($query)->links() }}
            <!-- /.box-body -->
          </div>
          <!-- /.box -->
        </div>
              ...
              <div class="col-md-12">
                ...
                <div class="col-md-8">
                  ...
                  <div class="form-group" id="otp_box" style="display: none;">
                    ...
                    <input type="text" name="otp_extend" id="otp_value" class="form-control number" >
