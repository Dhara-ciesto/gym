...
class EnrollController extends Controller
{
    ...
    public function enrollfingertemplate(Request $request){
    	...
    	$empid = $request->empid;
        $fingercount = $request->fingercount;
    	...
    	if(!empty($deviceinfo)){
            ...
            $fpdelete_api = 'http://'.$deviceip.':'.$port.'/device.cgi/credential?action=delete&user-id='.$empid.'&type=1';
            $apicronjob = new HRApiCronjob();
            $apicronjob->apiuserid = $empid;
            $apicronjob->apitype = 'Delete Fingertemplate';
            $apicronjob->api = $fpdelete_api;
            $apicronjob->response_code = 0;
            $apicronjob->status = 1;
            $apicronjob->save();
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_USERPWD, $username . ":" . $password);
            curl_setopt($ch, CURLOPT_URL,$fpdelete_api);
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
            $response = curl_exec($ch);
            $response = explode('=', $response);
            if($response[1] == 0){
                $finger = $fingercount-1;
                $fingerupload_api = 'http://'.$deviceip.':'.$port.'/device.cgi/enrolluser?action=enroll&user-id='.$empid.'&type=2&finger-count='.$finger;
                ...
                $response = explode('=', $response);
                 ...
                 if($response[1] == 0){
                         ...
                         while ($active)
                         {
                             if (time()>=$nexttick)
                             {
                                 return $this->getfingertemplate($apicronjob->apicronjobid,$fingercount,$empid,$deviceinfo);
                                 $active=false;
                             }
                         }
                     }else{
    ...
    public function getfingertemplate($apicronjoblastid,$fingercount,$employeeid,$deviceinfo){
        $deviceinfo = HRDeviceInfo::where('status', 1)->where('main', 1)->first();
    	...
    	if(!empty($deviceinfo)){
            ...
            $url = $deviceip.':'.$portno;
            $err=[];
            for ($i=1;$i<=$fingercount;$i++){

                $getfinger_api = 'http://'.$deviceip.':'.$portno.'/device.cgi/credential?action=get&type=1&user-id='.$employeeid.'&finger-index='.($i);
                    ...
                    if(strpos($response, 'Response-Code') === false){
                        ...
                        // if($fptemplateid[$i]) {
                        //     $fptemplate = FPtempleteModel::where('fptemplateid', $fptemplateid[$i])->update(['fptemplate' => $response]);
                        // }
                        $deviceuser = Device_Employee::where('employeeid', $employeeid)->first();
                        $deviceuser->istemplateupload=1; // Change status of Deviceuser
                        $deviceuser->save();
                        $fptemplate = new FPTemplate();
                        $fptemplate->employeeid = $employeeid;
                        $fptemplate->fingerprint = $response;
                        $fptemplate->save();
            ...
            if(in_array('1',$err)){
                ...
                $apicronjob_update = HRApiCronjob::findOrfail($apicronjoblastid);
                $apicronjob_update->response_code = 0;
                $apicronjob_update->status = 1;
                $apicronjob_update->save();
                return 201;
            }
    ...
    public function emplogajax(Request $request){
        ...
        if ($request->ajax()) {
           ...
           /* $query=[];
            ...
            $fetchlog = $fetchlog->select(DB::raw("CONCAT(employee.first_name,employee.last_name) AS fullname"), 'employee.mobileno', 'hr_device_emplog.dateid', 'hr_device_emplog.timein1','hr_device_emplog.timeout1','hr_device_emplog.timein2','hr_device_emplog.timeout2','hr_device_emplog.timein3','hr_device_emplog.timeout3')->orderby('hr_device_emplog.hr_device_emplogid','desc')->get();
/*
            dd($fetchlog->count());*/
