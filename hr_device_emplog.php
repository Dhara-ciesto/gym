...
$deviceevents=array();
    ...
    //  exit;
     foreach ($deviceevents as $key => $deviceevent) {
      ...
      $lastupdate =   strtotime("+15 minutes", strtotime($deviceevent['time']));
      echo $lastupdate;
      exit;
      if(in_array($lastupdate,$timeexist)){
        continue;
      }
       for ($i=1;$i<=3;$i++){
...
strtotime("+15 minutes", strtotime($startbreaktime)) >= strtotime($data[$i]['time']) && strtotime("-15 minutes", strtotime($startbreaktime)) <= strtotime($data[$i]['time'])
